...
// the server's `Status` message instead of a generic "Internal Server Error"
// string.
...
export const Platform: React.FC<PlatformProperties> = ({
  ...
  const handleSelectSite = useCallback((ref: ApiSiteRef) => {
    ...
    // `name` mirrors the site's display name, same as it does once the
    // platform is re-fetched from the server (see PlatformResources#map on
    // the Java side) — populate it locally too so the read-only "Name" field
    // isn't left blank until the next save/reload.
